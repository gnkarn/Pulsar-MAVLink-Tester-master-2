 ...
 */
// #ifndef Serial2
//     #define Serial2 Serial      // This is a complete hack because this file gets compiled no matter what I do, when I compile the AGT code.
//                             // And as the AGT does not have a Serial2 it fails to compile...even though we don't even call this function
//                             // from the AGT.
//                             // So to sort of fake it for the compiler I am just defining Serial2 to be Serial if Serial2 does not already exist.
//                             // On a Feather that has Serial2, my #define should be skipped.
// #endif
...
PulsarMavMissionResult_t mission_clear_all_ap()
{
    ...
    {
        Serial2.write(buf, len); // Write data to serial port byte by byte.
...
PulsarMavMissionResult_t mavlink_get_MISSION_ACK()
{
                                                         ...
                                                         //      we got the packet we were looking for.
    {
        ...
        while (Serial2.available() && !gotFullMsg) // xxx - I should prob put a time limiter on this WHILE, I think the only reason
